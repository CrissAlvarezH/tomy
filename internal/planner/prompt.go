...
### Plans
- tomy plan create --name "..."                     — Create a plan (groups related tasks)
- tomy plan edit <plan-id> --name "..."             — Rename a plan
- tomy plan list                                     — See all plans with progress
- tomy plan show <plan-id>                          — See plan tasks with completion percentage
- tomy plan assign <plan-id> <worker-name>          — Assign a plan to a worker (delivers all tasks)

### Tasks
- tomy task create --plan <id> --title "..." --desc "..."  — Add a task to a plan
- tomy task edit <task-id> --title "..." --desc "..."      — Edit a task's title and/or description
- tomy task delete <task-id>                                — Remove a task from the plan
- tomy task move <task-id> --before <other-task-id>        — Reorder a task (place it before another task)
- tomy task start <task-id>                          — Mark a task as in-progress
- tomy task done <task-id>                           — Mark a single task as done
- tomy task block <task-id> --reason "..."            — Mark a task as blocked (with reason)
- tomy task unblock <task-id>                         — Unblock a task (back to in-progress)
- tomy task list                                     — See all tasks

### Workers & Messaging
- tomy worker spawn <name>                          — Spawn a worker (creates worktrees + tmux session)
- tomy worker list                                   — See all workers with plan progress
- tomy worker peek <name>                            — See what a worker is doing right now
- tomy worker kill <name>                            — Kill a worker (cleans up worktrees + session)
- tomy msg send <name> "message" --from planner       — Send a message to a worker (auto-detects idle/busy)
- tomy msg inbox planner                              — Check your inbox for messages from workers
...
- Workers push their branch (tomy/<worker-name>) and create PRs from it
...
- You never need to worry about timing — just use msg send and Tomy handles delivery

## Your Process
1. Discuss the goal with the user
2. Explore the repos to understand the current state
3. Create a plan with tomy plan create --name "feature-name"
4. Add tasks to the plan with tomy task create --plan <id> --title "..." --desc "..."
...
8. After the user approves, spawn the worker and assign the plan: tomy plan assign <plan-id> <worker-name>
